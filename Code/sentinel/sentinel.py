...
# # List of cache servers (IDs and ports)
# CACHE_SERVERS = {
#     0: 50051,
#     1: 50052,
#     2: 50053
...
# def get_primary():
#     for server_id, port in CACHE_SERVERS.items():
#         try:
...
#                     print(f"Sentinel: CacheServer {server_id} is currently PRIMARY.")
#                     return server_id
#         except grpc.RpcError:
#             print(f"Sentinel: CacheServer {server_id} (port {port}) is DOWN.")
#     return None

# def get_most_updated_replica():
#     most_keys = -1
...
#         else:
#             print(f"Sentinel: Promotion failed for CacheServer {server_id}.")
#     except grpc.RpcError as e:
#         print(f"Sentinel: Failed to promote server {server_id}: {e}")
...
#         # Recheck current primary
#         still_alive = False
#         if current_primary is not None:
#             try:
#                 with grpc.insecure_channel(f'localhost:{CACHE_SERVERS[current_primary]}') as channel:
#                     stub = metadata_cache_channel_pb2_grpc.CacheServiceStub(channel)
#                     stub.GetLoad(metadata_cache_channel_pb2.EmptyRequest())  # any call to confirm health
#                     still_alive = True
#             except grpc.RpcError:
#                 print(f"Sentinel: Primary CacheServer {current_primary} is DOWN!")

#         if not still_alive:
#             print("Sentinel: Electing new primary...")
#             new_primary = get_most_updated_replica()
#             if new_primary:
#                 server_id, stub = new_primary
#                 promote(server_id, stub)
#                 current_primary = server_id
#             else:
#                 print("Sentinel: No replicas available to promote!")

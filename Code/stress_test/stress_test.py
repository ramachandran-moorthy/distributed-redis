...
def run_client(client_id):
    ...
    query_data = json.dumps({"id": student_id})
...
def main():
    ...
    # Spawn 200 clients, 50 at a time with 5-second gaps
    print("\n=== Running Client Load Test ===")
    total_clients = 0
    batch_size = 100
    
    for batch in range(4):  # 4 batches of 50 = 200 clients
        print(f"\nSpawning client batch {batch+1}/4 (clients {total_clients+1}-{total_clients+batch_size})...")
        
        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            # Submit 50 client tasks
            futures = []
        ...
        print(f"Batch {batch+1}/4 completed. Total clients processed: {total_clients}")
